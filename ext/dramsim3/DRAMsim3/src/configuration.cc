...
void Config::InitOtherParams(std::string config_file) {
    std::string is_write = "";
    if (config_file.find("write") != std::string::npos) {
        is_write = "write";
    } else {
        is_write ="";
    }
    ...
    output_prefix =
        output_dir + reader.Get("other", "output_prefix", "dramsim3") + is_write; //esj 2025-02-03
