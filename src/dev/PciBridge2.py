...
class PciBridge2(ClockedObject):
    type = "PciBridge2"
    # abstract = True
    cxx_class = "gem5::PciBridge2"
    cxx_header = "dev/PciBridge2.hh"
    rc_id = Param.UInt8(0, "Root Complex Id")
    rc_id_second = Param.UInt8(1, "Root Complex Id")
    rc_id_third = Param.UInt8(2, "Root Complex Id")
    rc_id_fourth = Param.UInt8(3, "Root Complex Id")
    ...
    pci_upstream_dev2 = Param.Int(
        0, "PCI device number of upstream switch port"
    )
    pci_upstream_func2 = Param.Int(
        0, "PCI func number of upstream switch port"
    )
    pci_upstream_dev3 = Param.Int(
        0, "PCI device number of upstream switch port"
    )
    pci_upstream_func3 = Param.Int(
        0, "PCI func number of upstream switch port"
    )
    pci_upstream_dev4 = Param.Int(
        0, "PCI device number of upstream switch port"
    )
    pci_upstream_func4 = Param.Int(
        0, "PCI func number of upstream switch port"
    )
    ...
    IOLimit = Param.UInt8(0x00, "Lower bits of IO limit")
    IOBase = Param.UInt8(0x00, "lower limits of IO Base")
    IOLimitUpper = Param.UInt16(0xFFFF, "Upper 16 bits of IO limit")
    IOBaseUpper = Param.UInt16(0x0000, "Upper 16 bits of IO Base")
    ...
    MSICAPBaseOffset = Param.UInt8(
        0x48, "Base offset of MSICAP in PCI Config space"
    )
    MSICAPCapId = Param.UInt8(
        0x05, " 0x48 Specifies this is the MSI Capability"
    )
    MSICAPNextCapability = Param.UInt8(
        0xD0, "0x49 Pointer to next capability block"
    )
    MSICAPMsgCtrl = Param.UInt16(0x018A, "0x4A MSI Message Control")
    MSICAPMsgAddr = Param.UInt32(0x00, "0x4C MSI Message Address")
    MSICAPMsgUpperAddr = Param.UInt32(0x00, "0x50 MSI Message Upper Address")
    MSICAPMsgData = Param.UInt16(0x00, "0x54 MSI Message Data")
    MSICAPMaskBits = Param.UInt32(0x00, "0x56    MSI Mask Bits")
    MSICAPPendingBits = Param.UInt32(0x00, "0x5A    MSI Pending Bits")

    # MSIXCAP Message Signaled Interrupt eXtended Capability
    MSIXCAPBaseOffset = Param.UInt8(0x60, "MSIXCAP capability base")
    MSIXCAPCapId = Param.UInt8(0x11, "0x60 MSIXCAP ID")
    MSIXCAPNextCapability = Param.UInt8(0xD0, "0x61 Next capability pointer")
    MSIXMsgCtrl = Param.UInt16(
        0x01FF, "0x62 MSI-X Message Control (512 vectors)"
    )
    MSIXTableOffset = Param.UInt32(
        0x00000004, "0x64 MSI-X Table Offset/BIR (Use BAR4)"
    )
    MSIXPbaOffset = Param.UInt32(
        0x00000005, "0x68 MSI-X PBA Offset/BIR (Use BAR5)"
    )

    ###Bridge related params
    response1 = ResponsePort("Response port")
    response2 = ResponsePort("Response port")
    ...
    # esj 2025-06-22
    request1_1 = RequestPort("Request port")
    request2_1 = RequestPort("Request port")
    request3_1 = RequestPort("Request Port")
    response1_1 = ResponsePort("Response port")
    response2_1 = ResponsePort("Response port")
    request1_2 = RequestPort("Request port")
    request2_2 = RequestPort("Request port")
    request3_2 = RequestPort("Request Port")
    response3 = ResponsePort("Response port")
    response4 = ResponsePort("Response port")
    response3_1 = ResponsePort("Response port")
    response4_1 = ResponsePort("Response port")

    request_dma1 = RequestPort("Request port")
    request_dma2 = RequestPort("Request port")
    request_dma3 = RequestPort("Request port")
    request_dma4 = RequestPort("Request port")
    ...
    cxl_chbs_base = Param.Addr(0x0, "CXL host bridge component register base")
    cxl_chbs_size = Param.UInt64(
        0x0, "CXL host bridge component register size"
    )
    cxl_hdm_base = Param.Addr(0x0, "CXL host bridge HDM decoder HPA base")
    cxl_hdm_size = Param.UInt64(0x0, "CXL host bridge HDM decoder HPA size")
    host_cxl_hdm_sizes = VectorParam.UInt64(
        [], "Per-host CXL host bridge HDM decoder HPA sizes"
    )

    # esj 2026-02-11
    routing_mode_type = Param.String("default", "Routing mode type")
    priority_limit = Param.Unsigned(10, "Priority limit")
    priority_host_ratios = Param.String(
        "10,0,0,0",
        "Weighted priority ratios per host as CSV (host1,host2,host3,host4)",
    )
...
class RootComplex(PciBridge2):
    ...
    PXCAPLinkCap = 0x01009102  # Max link width : 6'b1 , Max link speed : 2.5 Gbps , ASPM:00 , L0s exit latency: 64-128 ns , L1 exit latency 1-2 us. Port number : 1. # [0:3] link speed , [4:9] link width
    ...
    PXCAPLinkStatus = 0x0102  # Negotiated link width : 1 , Negotiated link speed : 2.5 Gbps # [0:3] link speed , [4:9] link width
