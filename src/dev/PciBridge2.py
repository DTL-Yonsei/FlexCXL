...
class PciBridge2(ClockedObject):
    ...
    PXCAPCapabilities = Param.UInt16(0x0001, "PCIe Capabilities")
    ...
    #     MSICAPBaseOffset = Param.UInt8(0x48, "Base offset of MSICAP in PCI Config space")
    #     MSICAPCapId = Param.UInt8(0x05, " 0x48 Specifies this is the MSI Capability")
    #     MSICAPNextCapability = Param.UInt8(0xD0, "0x49 Pointer to next capability block")
    ...
    #     MSIXMsgCtrl = Param.UInt16(0x01FF, "0x62 MSI-X Message Control (512 vectors)")
    #     MSIXTableOffset = Param.UInt32(0x00000004, "0x64 MSI-X Table Offset/BIR (Use BAR4)")
    #     MSIXPbaOffset = Param.UInt32(0x00000005, "0x68 MSI-X PBA Offset/BIR (Use BAR5)")
    ...
    response_1 = ResponsePort("Response port")
    ...
    validation_force_cxl_window = Param.Bool(
        False,
        "Validation-only equivalent of an enumerated upstream CXL BAR window",
    )
    validation_cxl_bar_start = Param.Addr(
        0x0, "Validation-only upstream CXL BAR window base"
    )
    validation_cxl_bar_size = Param.UInt64(
        0x0, "Validation-only upstream CXL BAR window size"
    )
...
class PCIESwitch(PciBridge2):
    ...
    CapabilityPointer = 0xD0  # 0x48 #esj 2025-04-2 change to 0xD0
    InterruptPin = 0x01
    InterruptLine = 0x20
    BridgeControl = 0x0000
    SecondaryStatus = 0x0000
