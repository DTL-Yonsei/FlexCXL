...
namespace gem5
{
    ...
    class CXLDRAMsim3 : public PciDevice {
        ...
        protected:
            ...
            std::array<uint8_t, 0x10000> cxlComponentRegs;
            std::array<uint8_t, 0x10000> cxlMemdevRegs;
            std::array<uint8_t, 4> cxlEventInterruptPolicy;
            ...
            struct CXLStats : public statistics::Group
            {
                ...
                statistics::Scalar requestAccepts;
                statistics::Scalar readAccepts;
                statistics::Scalar writeAccepts;
                statistics::Scalar admissionRejects;
                statistics::Scalar retryPendingRejects;
                statistics::Scalar globalOutstandingRejects;
                statistics::Scalar nativeQueueRejects;
                statistics::Scalar nativeWillAcceptRejectEpisodes;
                statistics::Scalar nativeWillAcceptRejectTicks;
                statistics::Scalar nativeWillAcceptRejectOpen;
                statistics::Scalar nativeReadQueueCurrent;
                statistics::Scalar nativeWriteQueueCurrent;
                statistics::Scalar nativeTotalQueueCurrent;
                statistics::Scalar nativeReadQueueMax;
                statistics::Scalar nativeWriteQueueMax;
                statistics::Scalar nativeTotalQueueMax;
                statistics::Scalar nativeReadQueueOccupancyTicks;
                statistics::Scalar nativeWriteQueueOccupancyTicks;
                statistics::Scalar nativeTotalQueueOccupancyTicks;
                statistics::Formula nativeReadQueueAverage;
                statistics::Formula nativeWriteQueueAverage;
                statistics::Formula nativeTotalQueueAverage;
                statistics::Scalar admissionStallEpisodes;
                statistics::Scalar admissionStallTicks;
                statistics::Scalar admissionStallOpen;
                statistics::Scalar dramSubmits;
                statistics::Scalar dramCompletions;
                statistics::Scalar dramRoiCompletions;
                statistics::Scalar dramCarryInAtReset;
                statistics::Scalar dramCarryInCompletions;
                statistics::Scalar dramUntrackedCompletions;
                statistics::Scalar dramRoiOutstandingCurrent;
                statistics::Scalar dramCarryInOutstandingCurrent;
                statistics::Scalar dramReadCompletions;
                statistics::Scalar dramWriteCompletions;
                statistics::Scalar backendResidenceSamples;
                statistics::Scalar backendResidenceTicks;
                statistics::Scalar backendResidenceMaxTicks;
                statistics::Scalar outstandingCurrent;
                statistics::Scalar outstandingMax;
                statistics::Scalar responseQueueCurrent;
                statistics::Scalar responseQueueMax;
                statistics::Scalar responseEnqueues;
                statistics::Scalar responseSends;
                statistics::Scalar responseBlockEpisodes;
                statistics::Scalar responseBlockTicks;
                statistics::Scalar responseBlockOpen;
            } stats;
        private:
        ...
        * @return pointer to the system object */
        ...
        /**
         * Explicit override for the endpoint global outstanding guard.
         * It is independent of optional event logging so logger-on/off runs
         * have identical admission behavior. Zero preserves the historical
         * behavior of using the native DRAMsim3 queue size.
         */
        const unsigned int validationQueueCapacity;

        unsigned int endpointQueueCapacity() const;
        void recordEndpointGlobalOccupancy(
            PacketPtr pkt, const std::string &direction,
            const std::string &reason);
        void recordEndpointRequestComplete(
            PacketPtr pkt, Addr backendAddr, unsigned int wrapperId = 0);
        void recordDramBoundaryProbe(
            PacketPtr pkt, Addr backendAddr, bool isWrite, bool accepted,
            const std::string &reason);
        void recordDramBoundaryEnqueue(
            PacketPtr pkt, Addr backendAddr, bool isWrite);
        void updateValidationOccupancyHighWatermarks();
        void beginAdmissionStall();
        void endAdmissionStall();
        void updateNativeQueueOccupancy();
        void resetNativeQueueOccupancyStats();
        void beginNativeWillAcceptStall(Addr addr, bool is_write);
        void refreshNativeWillAcceptStall();
        void accrueNativeWillAcceptStall();

        uint64_t endpointOutstandingHighWatermark = 0;
        uint64_t endpointResponseQueueHighWatermark = 0;
        uint64_t backendResidenceHighWatermark = 0;
        bool endpointAdmissionStallActive = false;
        Tick endpointAdmissionStallStart = 0;
        bool endpointResponseBlockActive = false;
        Tick endpointResponseBlockStart = 0;

        uint64_t nativeReadQueueOccupancy = 0;
        uint64_t nativeWriteQueueOccupancy = 0;
        uint64_t nativeTotalQueueOccupancy = 0;
        uint64_t nativeReadQueueHighWatermark = 0;
        uint64_t nativeWriteQueueHighWatermark = 0;
        uint64_t nativeTotalQueueHighWatermark = 0;
        Tick nativeQueueOccupancyLastUpdate = 0;

        bool nativeWillAcceptStallActive = false;
        Tick nativeWillAcceptStallStart = 0;
        Addr nativeWillAcceptBlockedAddr = 0;
        bool nativeWillAcceptBlockedIsWrite = false;

        /** Monotonic ordering for observational DRAM-boundary events. */
        uint64_t validationDramBoundaryOrder = 0;
        uint64_t validationDramCompletionOrder = 0;

        struct DramSubmitRecord
        {
            Tick submitTick = 0;
            /** False for a transaction already in flight at stats reset. */
            bool roiEligible = true;
        };

        /** DRAM submit state keyed by the stable Request object. */
        std::unordered_map<const Request *, DramSubmitRecord> dramSubmitTicks;

        /** Validation metadata; never consulted when admitting traffic. */
        std::unordered_map<const Request *, uint64_t>
            validationAdmissionAttempts;

        struct ValidationBlockedRequest
        {
            bool valid = false;
            ...
            int64_t queueOccupancy = 0;
            int64_t queueCapacity = 0;
            bool isControl = false;
            bool crcOk = false;
            uint64_t requestSize = 0;
            bool isWrite = false;
        } validationBlockedRequest;

        /** Port-level attempts to transmit the head response packet. */
        uint64_t validationResponseAttempt = 0;

        void initCxlRegisterBlocks();
        bool cxlRegisterOffset(Addr addr, bool &memdev, Addr &offset);
        bool accessCxlRegisterMmio(PacketPtr pkt);
        bool recvCxlRegisterTimingReq(PacketPtr pkt);
        void writeCxlRegisterBytes(bool memdev, Addr offset,
                                   const uint8_t *data, unsigned size);
        void executeCxlMailboxCommand();
        ...
        /** Earliest response-send tick for each queued packet. */
        std::unordered_map<PacketPtr, Tick> responseReadyTicks;
