...
namespace gem5
{
    ...
    CXL_Packing::CXL_PackingRequestPort::CXL_PackingRequestPort(
        const std::string& _name, CXL_Packing* packer)
        : RequestPort(_name, packer), packer(packer)
    {}


    CXL_Packing::CXL_PackingResponsePort::CXL_PackingResponsePort(
        const std::string& _name, CXL_Packing* packer)
    ...
    CXL_Packing::packing_packet(PacketPtr packet){
        ...
        if(!packet->cxl_pkt.is_packed & packet->is_cxl_mem){
            if(packet->cxl_pkt.isReq | packet->cxl_pkt.isNDR){
                ...
                DPRINTF(CXL_Packing,
                        "%s isReq? = %d, isNDR?= %d, packing_size = %d, "
                        "flit_num = %d, delay = %d\n",
                        __func__, packet->cxl_pkt.isReq,
                        packet->cxl_pkt.isNDR,
                        packet->cxl_pkt.packing_size,
                        packet->cxl_pkt.flit_num, delay);
            }
                ...
                if (size <= 48){
                    ...
                    packet->cxl_pkt.packing_size =
                        64 * packet->cxl_pkt.flit_num;
                    delay = cycleperflit*this->clockPeriod();
                }
                // pkt->packing_size += (2*pkt->flit_num); //protocol id add
                DPRINTF(CXL_Packing,
                        "%s isRwD? = %d, isDRS?= %d, packing_size = %d, "
                        "flit_num = %d, delay = %d\n",
                        __func__, packet->cxl_pkt.isRwD,
                        packet->cxl_pkt.isDRS,
                        packet->cxl_pkt.packing_size,
                        packet->cxl_pkt.flit_num, delay);
    ...
    CXL_Packing::trySendTiming()
    {
        ...
        if (hasRetryPriority(pkt)) {
            DPRINTF(CXL_Packing,
                    "[%s] H2D priority host=%u seq=%llu index=%llu\n",
                    isRetryControl(pkt) ? "LRSM" : "RRSM",
                    static_cast<unsigned>(pkt->source_host_idx),
    ...
    CXL_Packing::trySendTimingResp(){
        ...
        if (hasRetryPriority(pkt)) {
            DPRINTF(CXL_Packing,
                    "[%s] D2H priority host=%u seq=%llu index=%llu\n",
                    isRetryControl(pkt) ? "LRSM" : "RRSM",
                    static_cast<unsigned>(pkt->source_host_idx),
                    static_cast<unsigned long long>(pkt->cxl_pkt.seqNum),
                    static_cast<unsigned long long>(selected_index));
