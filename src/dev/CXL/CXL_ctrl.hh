...
namespace gem5
{
    ...
    public:
        ...
        PacketPtr peekByseqnum(uint64_t seq_num) const;
        ...
        using OccupancyObserver = std::function<void(uint64_t)>;
        OccupancyObserver occupancyObserver;

        void setOccupancyObserver(OccupancyObserver observer)
        {
            occupancyObserver = std::move(observer);
        }

        void notifyOccupancyIfChanged(size_t old_size)
        {
            if (occupancyObserver && old_size != queue.size())
                occupancyObserver(queue.size());
        }
    ...
    class CXL_ctrl : public ClockedObject //SimObject
    {
        ...
        struct CXLStats : public statistics::Group
        {
            ...
            statistics::Scalar ack_control_flit_count;
            statistics::Scalar nak_control_flit_count;
            statistics::Scalar retry_req_count;
            statistics::Scalar retry_resp_count;
            statistics::Scalar retry_ack_count;
            statistics::Scalar retransmit_req_count;
            statistics::Scalar retransmit_resp_count;
            statistics::Scalar seq_timeout_req_count;
            statistics::Scalar seq_timeout_resp_count;
            statistics::Scalar replay_buffer_high_watermark;
            statistics::Scalar ack_rx_count;
            statistics::Scalar ack_rx_h2d_count;
            statistics::Scalar ack_rx_d2h_count;
            statistics::Scalar nak_rx_count;
            statistics::Scalar nak_rx_h2d_count;
            statistics::Scalar nak_rx_d2h_count;
            statistics::Scalar ack_piggyback_count;
            statistics::Scalar nak_piggyback_count;
            statistics::Scalar standalone_ack_tx_count;
            statistics::Scalar nak_tx_count;
            statistics::Scalar control_flit_tx_count;
            statistics::Scalar cft_arm_count;
            statistics::Scalar cft_cancel_count;
            statistics::Scalar cft_non_piggyback_cancel_count;
            statistics::Scalar cft_expiry_count;
            statistics::Scalar exact_retry_req_tx_count;
            statistics::Scalar exact_retry_resp_tx_count;
            statistics::Scalar replay_admission_stall_episodes;
            statistics::Scalar replay_admission_stall_ticks;
            statistics::Scalar replay_admission_stall_open;
            statistics::Scalar host_replay_current;
            statistics::Scalar device_replay_current;
            statistics::Scalar host_replay_max;
            statistics::Scalar device_replay_max;
            statistics::Scalar host_replay_occupancy_ticks;
            statistics::Scalar device_replay_occupancy_ticks;
            statistics::Formula host_replay_average;
            statistics::Formula device_replay_average;
            statistics::Scalar host_replay_carry_in_at_reset;
            statistics::Scalar device_replay_carry_in_at_reset;
            statistics::Scalar cft_pending_current;
            statistics::Scalar cft_h2d_pending_current;
            statistics::Scalar cft_d2h_pending_current;
            statistics::Scalar cft_pending_carry_in_at_reset;
            statistics::Scalar cft_h2d_pending_carry_in_at_reset;
            statistics::Scalar cft_d2h_pending_carry_in_at_reset;
            statistics::Scalar admission_stall_host_replay_count;
            statistics::Scalar admission_stall_host_tx_queue_count;
            statistics::Scalar admission_stall_host_recv_window_count;
            statistics::Scalar admission_stall_device_replay_count;
            statistics::Scalar admission_stall_device_tx_queue_count;
            statistics::Scalar admission_stall_read_reservation_count;
            statistics::Scalar host_outstanding_window_current;
            statistics::Scalar host_outstanding_window_max;
            statistics::Scalar host_outstanding_window_occupancy_ticks;
            statistics::Formula host_outstanding_window_average;
            statistics::Scalar host_outstanding_window_carry_in_at_reset;
            statistics::Scalar host_outstanding_window_stall_episodes;
            statistics::Scalar host_outstanding_window_stall_ticks;
            statistics::Scalar host_outstanding_window_stall_open;
            statistics::Scalar host_deferred_tx_current;
            statistics::Scalar host_deferred_tx_max;
            statistics::Scalar host_deferred_tx_occupancy_ticks;
            statistics::Formula host_deferred_tx_average;
            statistics::Scalar host_deferred_tx_carry_in_at_reset;
        ...
        ///
        mutable uint64_t replayBufferHighWatermark = 0;
        ...
        void syncStrictCftReqEvent();
        void syncStrictCftRespEvent();
        bool popfromtransmitlist_req(
            uint64_t seq_num, bool carried_by_data = false);
        bool popfromtransmitlist_resp(
            uint64_t seq_num, bool carried_by_data = false);
        ...
        // A D2H packet continues to the host after it is admitted to the
        // replay buffer.  Keep a controller-owned snapshot rather than an
        // alias of that in-flight packet, whose receiver owns its lifetime.
        ...
        int maxQueueSize;
        bool validationReplayPolicyEnable;
        std::string validationReplayPolicy;
        int validationRbStallThreshold;
        int validationRbResumeThreshold;
        bool validationStrictCftDeadline;
        int validationOutstandingWindowCapacity;
        bool hostRbStallActive = false;
        Tick hostRbStallStart = 0;
        bool hostOutstandingWindowStallActive = false;
        Tick hostOutstandingWindowStallStart = 0;

        struct ReplayOccupancyTracker
        {
            Tick lastUpdate = 0;
            uint64_t current = 0;
            uint64_t maximum = 0;
            uint64_t occupancyTicks = 0;
            uint64_t carryInAtReset = 0;
        };

        ReplayOccupancyTracker hostReplayOccupancy;
        ReplayOccupancyTracker deviceReplayOccupancy;
        ReplayOccupancyTracker hostOutstandingWindowOccupancy;
        ReplayOccupancyTracker hostDeferredTxOccupancy;
        void updateReplayOccupancy(bool host, uint64_t occupancy);
        void resetReplayOccupancyTracking();
        void updateHostOutstandingWindowOccupancy(uint64_t occupancy);
        void resetHostOutstandingWindowTracking();
        int hostOutstandingWindowCapacity() const;
        void recordHostOutstandingWindowChange(
            PacketPtr pkt, const std::string &event_name,
            const std::string &reason);
        void beginHostOutstandingWindowStall(PacketPtr pkt);
        void endHostOutstandingWindowStallIfReady(PacketPtr pkt);
        void updateHostDeferredTxOccupancy(uint64_t occupancy);
        void resetHostDeferredTxTracking();
        void recordHostDeferredTxChange(
            PacketPtr pkt, const std::string &event_name,
            const std::string &reason);
        uint64_t cftReqPending() const;
        uint64_t cftRespPending() const;
        uint64_t hostReplayReservationWeight(PacketPtr pkt) const;
        uint64_t hostReplayReservedOccupancy();
        bool hostReplayBlocksAdmission(PacketPtr pkt);
        int hostRbStallThreshold() const;
        int hostRbResumeThreshold() const;
        std::string hostRbPolicyLabel() const;
        void beginHostRbStall(PacketPtr pkt);
        void endHostRbStallIfReady(PacketPtr pkt);
        void countAckRx(bool d2h, uint64_t count = 1);
        void countNakRx(bool d2h, uint64_t count = 1);
