...
namespace gem5
{
    ...
    CXL_CRC_check::CRC_packet_check(PacketPtr pkt){
        DPRINTF(CXL_CRC_check,"%s, pkt addr = %0x, packed = %d\n",__func__,pkt->getAddr(),pkt->cxl_pkt.is_packed);
        ...
        if(pkt->cxl_pkt.crc_check){
            //esj 2025-11-29
            // delay += cycleperflit*pkt->cxl_pkt.flit_num*this->clockPeriod();
            DPRINTF(CXL_CRC_check, "%s, crc_data = %lf, success_rate = %lf\n",__func__, pkt->cxl_pkt.crc_data, success_rate);
            //esj 2025-05-20
            // if(pkt->cxl_pkt.crc_data >= success_rate | pkt->cxl_pkt.is_controlflit
            //esj 2025-07-18 add | -> ||
            // if(pkt->cxl_pkt.crc_data_double >= success_rate || pkt->cxl_pkt.is_controlflit || pkt->is_cxl_write_resp //esj 2025-11-21
            if(pkt->cxl_pkt.crc_data >= success_rate || pkt->cxl_pkt.is_controlflit || pkt->is_cxl_write_resp //esj 2025-11-21

            //esj 2025-01-18 add crc_error
            || pkt->cxl_pkt.crc_error_req_seqnum!=-1 || pkt->cxl_pkt.crc_error_resp_seqnum!=-1){ //esj 2024-12-13 add control flit case
        ...
        if (!pkt->cxl_pkt.crc_check && !pkt->cxl_pkt.is_controlflit) {
            ProtocolValidationEvent detected;
            detected.event = "CRC_ERROR_DETECTED";
            detected.component = name();
            detected.layer = "CXL_PROTOCOL";
            detected.direction =
                pkt->cxl_pkt.is_from_device ? "D2H" : "H2D";
            detected.packetSeq = pkt->cxl_pkt.seqNum;
            detected.flitIndex = 0;
            detected.txAttempt =
                (pkt->cxl_pkt.retry_req || pkt->cxl_pkt.retry_resp ||
                 pkt->cxl_pkt.is_retry_req ||
                 pkt->cxl_pkt.is_retry_resp) ? 1 : 0;
            detected.reason = "CRC_CHECK_FAILED";
            ProtocolValidationLogger::recordPacket(detected, pkt);
        }
