...
namespace gem5
{
    CXLSwitch::CXLResponsePort::CXLResponsePort(const std::string& _name,
                                         CXLSwitch& _bridge,
                                         bool upstream , 
                                         uint8_t _respID , 
                                         uint8_t _rcid , 
...
CXLSwitch::CXLRequestPort::CXLRequestPort(const std::string& _name,
                                           CXLSwitch& _bridge, uint8_t _rcid , 
                                           Cycles _delay, 
                                        //    Tick _delay, 
                                           int _req_limit)
...
void CXLSwitch::initialize_ports ( config_class * storage_ptr , const CXLSwitchParams &p)
{
...
CXLSwitch::CXLSwitch(const CXLSwitchParams &p)
    :ClockedObject(p),
    //esj 2025-04-21
    responsePort1(p.name + ".response1", *this, true,0, p.rc_id , ticksToCycles(p.delay), p.resp_size),
    responsePort2(p.name + ".response2", *this, true,0, p.rc_id , ticksToCycles(p.delay), p.resp_size),
...
Addr config_class::getBar0()
{
    ...
    //DPRINTF(CXLSwitch, "esj getbar0 before storage1.bar0 = %x/n",storage1.bar0);
    bar0 = bar0 & ~(barsize[0] - 1) ; 
    //DPRINTF(CXLSwitch, "esj getbar0 bar0 = %x/n",bar0);
...
Addr config_class::getBar1()
{
    ...
    //DPRINTF(CXLSwitch, "esj getbar1 before storage1.bar1 = %x/n",storage1.Bar1);
    bar1 = bar1 & ~(barsize[1] - 1 ) ; 
    //DPRINTF(CXLSwitch, "esj getbar1 bar1 = %x/n",bar1);
...
CXLSwitch::CXLRequestPort*
CXLSwitch::getRequestPort ( Addr address)
{
    if((address >= storage_ptr1->getMemoryBase()) && (address <= storage_ptr1->getMemoryLimit()) && (storage_ptr1->getMemoryBase()!=0) &&(storage_ptr1->getMemoryLimit()!=0))
        return (CXLSwitch::CXLRequestPort*) &requestPort1 ;
    ...
    if((address >= storage_ptr2->getMemoryBase()) && (address <= storage_ptr2->getMemoryLimit()) && (storage_ptr2->getMemoryBase()!=0) &&(storage_ptr2->getMemoryLimit()!=0))
        return (CXLSwitch::CXLRequestPort*) &requestPort2 ;
    ...
    if((address >= storage_ptr3->getMemoryBase()) && (address <= storage_ptr3->getMemoryLimit()) && (storage_ptr3->getMemoryBase()!=0) &&(storage_ptr3->getMemoryLimit()!=0))
        return (CXLSwitch::CXLRequestPort*) &requestPort3 ;
    ...
    return (CXLSwitch::CXLRequestPort*) &requestPort_DMA ; // If no downstream ports are willing to handle the request , just give it to the DMA Port 
}
 
CXLSwitch::CXLResponsePort*

CXLSwitch::getResponsePort(int bus_num)
{
    if ( (bus_num >= storage_ptr1->storage1.SecondaryBusNumber) && (bus_num <=storage_ptr1->storage1.SubordinateBusNumber) && (storage_ptr1->is_valid == 1))
        return (CXLResponsePort*)&responsePort_DMA1 ; 
    else if ( (bus_num >= storage_ptr2->storage1.SecondaryBusNumber) && (bus_num <= storage_ptr2->storage1.SubordinateBusNumber) && (storage_ptr2->is_valid ==1))
    {
        ...
        return (CXLResponsePort*) &responsePort_DMA2 ;
    } 
        ...
        return (CXLResponsePort*) &responsePort_DMA3 ;

    //printf("Returning upstream responsePort1 for bus num %d\n" , bus_num) ; 
    return (CXLResponsePort*)&responsePort1 ; 
}
...
//   DPRINTF(CXLSwitch, "esj read pkt->getSize()pkt->getSize()pkt->getSize() = %d\n", pkt->getSize());
...
Tick config_class::writeConfig(PacketPtr pkt)
{
    ...
    if( offset < PCIE_HEADER_SIZE )
    {
        ...
        else
        {
            switch(pkt->getSize())
            {
                case sizeof(uint8_t):  DPRINTF(CXLSwitch,"pcibridge writeConfig: dev %#x reg %#x 1 bytes: data = %#x\n",storage1.DeviceId, offset,(uint8_t)pkt->get<uint8_t>(ByteOrder::little));
                                        ...
                                        break ; 
                case sizeof(uint16_t): DPRINTF(CXLSwitch,"pcibridge writeConfig: dev %#x reg %#x 16bit: data = %#x\n",storage1.DeviceId, offset,(uint16_t)pkt->get<uint16_t>(ByteOrder::little));                               
                                        ...
                                        else if (offset == PCI1_MEM_BASE)
                                        {
                                            ...
                                            //DPRINTF(CXLSwitch, "esj 16bit pcibridge memory base = %x\n",storage1.MemoryBase);
                                            flag =1 ; 
                                        ...
                                        else if (offset == PCI1_MEM_LIMIT)
                                        {
                                            ...
                                            // DPRINTF(CXLSwitch, "esj 16bit pcibridge Memory Limit = %x\n",storage1.MemoryLimit);
                                            flag = 1 ;
                ...
                case sizeof(uint32_t): DPRINTF(CXLSwitch,"pcibridge writeConfig: dev %#x reg %#x 32bit: data = %#x\n",storage1.DeviceId, offset,(uint32_t)pkt->get<uint32_t>(ByteOrder::little));                                                            
                // Can be expansion ROM Base Address , BAR0 , BAR1 or PrefetchableBase/Limit Upper
                                    ...
                                    else if (offset == PCI1_BASE_ADDR0)
                                    {
                                        ...
                                        //DPRINTF(CXLSwitch, "esj PCI1_BASE_ADDR0 = %x\n",val);
                                    ...
                                    else if (offset == PCI1_BASE_ADDR1)
                                    {
                                        ...
                                        int val = letoh(pkt->get<uint32_t>(ByteOrder::little)) ;
                                        //DPRINTF(CXLSwitch, "esj PCI1_BASE_ADDR1 = %x\n",val);
                                        uint32_t Bar1 = letoh(storage1.Bar1) ;
...
CXLSwitch::init()
{
    // make sure both sides are connected and have the same block size
     if (!responsePort1.isConnected() || !responsePort_DMA1.isConnected() || !responsePort_DMA2.isConnected() || !responsePort_DMA3.isConnected() || !requestPort_DMA.isConnected() || !requestPort1.isConnected() || !requestPort2.isConnected() || !requestPort3.isConnected())
...
bool
CXLSwitch::CXLResponsePort::respQueueFull() const
{
    return outstandingResponses == respQueueLimit;
} 

bool
CXLSwitch::CXLRequestPort::reqQueueFull() const
{
    return transmitList.size() == reqQueueLimit;
}

bool
CXLSwitch::CXLRequestPort::recvTimingResp(PacketPtr pkt)
{
    // all checks are done when the request is accepted on the response
    // side, so we are guaranteed to have space for the response
    DPRINTF(CXLSwitch, "recvTimingResp: %s addr 0x%x\n",
            pkt->cmdString(), pkt->getAddr());
    DPRINTF(CXLSwitch, "CXLRequestPort::recvTimingResp: pkt addr = %x\n", pkt->getAddr());
    DPRINTF(CXLSwitch, "Request queue size: %d\n", transmitList.size());
   // if((pkt->req_bus[rcid] == 0) && (bridge.is_switch == 1)) printf("Request Port received response to switch port\n"); 
    CXLResponsePort * responsePort1 = bridge.getResponsePort(pkt->req_bus[rcid]) ;
   // if(bridge.is_switch == 1) printf("Found response %d\n" , responsePort1->respID) ; 
    if((responsePort1->respID == 1) && (bridge.is_switch==0) && (pkt->isResponse()) && (pkt->hasData()) && (bridge.is_transmit == 1))totalCount += pkt->getSize() ;  
    ...
    responsePort1->schedTimingResp(pkt, bridge.clockEdge(delay) +receive_delay); //esj 2024-04-16
    //esj 2024-12-27
    DPRINTF(CXLSwitch,"%s, recv response packet addr = %0x, when = %d\n",__func__,pkt->getAddr(),bridge.clockEdge(delay) +receive_delay);
...
bool
CXLSwitch::CXLResponsePort::recvTimingReq(PacketPtr pkt)
{
   Addr pkt_Addr = pkt->getAddr() ; 
   //if((respeID==0) &&(bridge.is_switch==0))printf("Response received timing req to Addr: %08x\n" , (unsigned int)pkt_Addr) ; 
   DPRINTF(CXLSwitch, "CXLResponsePort::recvTimingReq: pkt addr = %x\n", pkt->getAddr());
   CXLRequestPort * requestPort  = bridge.getRequestPort(pkt_Addr) ; 
   if(pkt->req_bus[rcid] == -1) // response port assigns the requester id
   {
    ...
    DPRINTF(CXLSwitch, "recvTimingReq: %s addr 0x%x\n",
            pkt->cmdString(), pkt->getAddr());
    ...
    DPRINTF(CXLSwitch, "Response queue size: %d pkt->resp: %d\n",
            transmitList.size(), outstandingResponses);

    // if the request queue is full then there is no hope
    if (requestPort->reqQueueFull()) {
        DPRINTF(CXLSwitch, "Request queue full\n");
        retryReq = true;
    } else {
        ...
        if (expects_response) {
            if (respQueueFull()) {
                DPRINTF(CXLSwitch, "Response queue full\n");
                retryReq = true;
            } else {
                // ok to send the request with space for the response
                DPRINTF(CXLSwitch, "Reserving space for response\n");
                assert(outstandingResponses != respQueueLimit);
                ++outstandingResponses;
                DPRINTF(CXLSwitch, "CXLResponsePort::recvTimingReq outstandingResponses = %d\n",outstandingResponses);
        ...
        if (!retryReq) {
            ...
            Tick receive_delay = pkt->headerDelay + pkt->payloadDelay;
            // DPRINTF(CXLSwitch, "%s:pkt addr : %0x, pkt->headerDelay = %d,  pkt->payloadDelay = %d, Flexbus_delay = %d, receive_delay = %d\n",__func__,pkt->headerDelay, pkt->payloadDelay,Flexbus_delay,receive_delay);
            // Tick receive_delay = pkt->headerDelay + pkt->payloadDelay;
            pkt->headerDelay = pkt->payloadDelay = 0;
...
void
CXLSwitch::CXLResponsePort::retryStalledReq()
{
    if (retryReq) {
        DPRINTF(CXLSwitch, "Request waiting for retry, now retrying\n");
        retryReq = false;
        sendRetryReq();
    }
}
...
//esj 2024-05-08
Tick 
CXLSwitch::CXLResponsePort::Flex_Bus(PacketPtr pkt){
...
void
CXLSwitch::CXLRequestPort::schedTimingReq(PacketPtr pkt, Tick when)
{
    ...
    //     DPRINTF(CXLSwitch, "esj CXLSwitch::PciBridgeRequestPort::schedTimingReq when = %d\n",when);
    ...
    DPRINTF(CXLSwitch, "PciBridgeRequestPort::schedTimingReq: pkt addr = %x, when = %d\n", pkt->getAddr(),when);
    transmitList.emplace_back(pkt, when);
    DPRINTF(CXLSwitch, "PciBridgeRequestPort::schedTimingReq: transmitList[0].pkt->getAddr() = %x, cmd = %s\n", pkt->getAddr(), pkt->cmd.toString());
}


void
CXLSwitch::CXLResponsePort::schedTimingResp(PacketPtr pkt, Tick when)
{
    // If we're about to put this packet at the head of the queue, we
    // need to schedule an event to do the transmit.  Otherwise there
    // should already be an event scheduled for sending the head
    // packet.
    // if (transmitList.empty()) {
    //     DPRINTF(CXLSwitch, "esj CXLSwitch::CXLResponsePort::schedTimingResp when = %d, pkt->addr: %x, pkt->isresponse = %d\n",when, pkt->getAddr(),pkt->isResponse());
    //     bridge.schedule(sendEvent, when);
    // }
    ...
    DPRINTF(CXLSwitch, "CXLResponsePort::schedTimingResp: pkt addr = %x\n", pkt->getAddr());
    transmitList.emplace_back(pkt, when);
    DPRINTF(CXLSwitch, "CXLResponsePort::schedTimingResp: addr = %x, cmd = %s, when = %d\n", pkt->getAddr(),pkt->cmd.toString(),when);
    
}

void
CXLSwitch::CXLRequestPort::trySendTiming()
{
    assert(!transmitList.empty());

    DeferredPacket req = transmitList.front();
    DPRINTF(CXLSwitch, "CXLRequestPort::trySendTiming: transmitList[0].pkt->getAddr() = %x, cmd = %s\n", transmitList[0].pkt->getAddr(),transmitList[0].pkt->cmd.toString());
    assert(req.tick <= curTick());

    PacketPtr pkt = req.pkt;
   
    DPRINTF(CXLSwitch, "trySend request addr 0x%x, queue size %d\n",
            pkt->getAddr(), transmitList.size());

    if (sendTimingReq(pkt)) {
        // send successful
        transmitList.pop_front();
        DPRINTF(CXLSwitch, "trySend request successful\n");

        // If there are more packets to send, schedule event to try again.
        if (!transmitList.empty()) {
            DeferredPacket next_req = transmitList.front();
            DPRINTF(CXLSwitch, "Scheduling next send\n");
...
void CXLSwitch::CXLRequestPort:: incCount()
{
...
void
CXLSwitch::CXLResponsePort::trySendTiming()
{
    
    assert(!transmitList.empty());

    DeferredPacket resp = transmitList.front();
    DPRINTF(CXLSwitch, "CXLResponsePort::trySendTiming: transmitList[0].pkt->getAddr() = %x, size = %d, cmd = %s\n", transmitList[0].pkt->getAddr(),transmitList.size(),transmitList[0].pkt->cmd.toString());
    assert(resp.tick <= curTick());

    PacketPtr pkt = resp.pkt;
  //  if((respID==0) && (bridge.is_switch == 0)) printf("Upstream switch port received response\n") ; 

    DPRINTF(CXLSwitch, "trySend response addr 0x%x, outstanding %d\n",
            pkt->getAddr(), outstandingResponses);

    if (sendTimingResp(pkt)) {
        // send successful
        DPRINTF(CXLSwitch, "CXLResponsePort::trySendTiming: before pop front , transmitlist size = %d\n",transmitList.size());
        transmitList.pop_front();
        DPRINTF(CXLSwitch, "trySend response successful\n");
        DPRINTF(CXLSwitch, "CXLResponsePort::trySendTiming: after pop front , transmitlist size = %d\n",transmitList.size());
        //////////////////////// esj 2023-04-24
        assert(outstandingResponses != 0);
        --outstandingResponses;
        DPRINTF(CXLSwitch, "CXLResponsePort::trySendTiming --outstandingResponses = %d\n",outstandingResponses);
        ////////////////////////////
...
void
CXLSwitch::CXLRequestPort::recvReqRetry()
{
    DPRINTF(CXLSwitch, "CXLRequestPort::recvReqRetry\n");
    trySendTiming();
}

void
CXLSwitch::CXLResponsePort::recvRespRetry()
{
    DPRINTF(CXLSwitch, "CXLResponsePort::recvReqRetry\n");
    trySendTiming();
}

Tick
CXLSwitch::CXLResponsePort::recvAtomic(PacketPtr pkt)
{
    ...
    if(pkt->req_bus[rcid] == -1) // response port assigns the requester id
   {
    ...
    DPRINTF(CXLSwitch, "CXL ::recvAtomic: pkt addr = %x, pkt = %s\n", pkt->getAddr(), pkt->print());
    CXLRequestPort * requestPort = bridge.getRequestPort(pkt->getAddr()) ; 
    panic_if(pkt->cacheResponding(), "Should not see packets where cache "
             "is responding");
    ...
    if(bridge.is_switch)
        latency += delay;
        if(pkt->cxl_flag) 
            DPRINTF(CXLSwitch,"esj %s latenncy = %d, latency+50ns = %d\n",__func__,latency,latency+delay);
...
void
CXLSwitch::CXLResponsePort::recvFunctional(PacketPtr pkt)
{
...
bool
CXLSwitch::CXLRequestPort::checkFunctional(PacketPtr pkt)
{
    bool found = false;
...
void 
CXLSwitch::CXLResponsePort::fill_ranges(AddrRangeList & ranges , config_class * storage_ptr) const 
{
    //esj 2024-02-16
    uint32_t barsize[2] ; 
    //uint64_t barsize[2] ; 
    ...
    //flag2 = 0 ; 
    //flag3 = 0 ;  
    //DPRINTF(CXLSwitch, "esj fill_range ");
    //DPRINTF(CXLSwitch, "esj Memory base is %0x , memory limit is %0x\n" , MemoryBase, MemoryLimit) ;
    //DPRINTF(CXLSwitch, "esj Prefetch Memory Base is %0x , Prefetch MEmory Limit is %0x\n" , (unsigned int) PrefetchableMemoryBase , (unsigned int) PrefetchableMemoryLimit) ;  
    if(barsize[0]!=0 && Bar0!=0)
    {
        DPRINTF(CXLSwitch, "Bar0 assigned\n") ; 
    ...
    {
        //flag2 = 1 ; 
        DPRINTF(CXLSwitch, "Prefetch Memory Base is %0x , Prefetch MEmory Limit is %0x\n" , (unsigned int) PrefetchableMemoryBase , (unsigned int) PrefetchableMemoryLimit) ;  
    ...
    {
        //flag3 = 1 ;
        DPRINTF(CXLSwitch, "pushing back addr io ranges %0x - %0x\n" , (unsigned int)IOBase ,(unsigned int) IOLimit) ;     
...
AddrRangeList
CXLSwitch::CXLResponsePort::getAddrRanges() const
{
    ...
    else
    {
        ...
        for (auto it = ranges.begin() ; it !=ranges.end() ; it++)
        {
            ...
            //DPRINTF(CXLSwitch, "esj CXL last_val = %x\n",last_val);
            last_val = (*it).end() + 1 ;
        }
...
void
CXLSwitch::serialize(CheckpointOut &cp) const
{
...
void
CXLSwitch::unserialize(CheckpointIn &cp)
{
...
CXLSwitch::CXLRequestPort&
CXLSwitch::getRequestPort(const std::string &if_name, PortID idx)
{
    if (if_name == "request1")
        return requestPort1;
...
CXLSwitch::CXLResponsePort&
CXLSwitch::getResponsePort(const std::string &if_name, PortID idx)
{
    if (if_name == "response")
